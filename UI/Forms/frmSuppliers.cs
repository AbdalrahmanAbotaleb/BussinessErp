...
namespace BussinessErp.UI.Forms
{
    public class frmSuppliers : Form
    {
        ...
        private SupplierService _svc = new SupplierService();
        ...
        {
            this.Text = LanguageManager.Get("nav_suppliers");
            this.BackColor = Color.FromArgb(243, 244, 246);
            this.Padding = new Padding(20);

            LanguageManager.ApplyRTL(this);

            var lblTitle = new Label { Text = "🏭 " + LanguageManager.Get("nav_suppliers"), Font = UIHelper.FontTitle, ForeColor = UIHelper.TextDark, Dock = DockStyle.Top, Height = 45 };
            ...
            dgv.SelectionChanged += (s, e) => {
                if (dgv.CurrentRow?.DataBoundItem is Supplier sup)
                { _selectedId = sup.Id; txtName.Text = sup.Name; txtPhone.Text = sup.Phone; txtEmail.Text = sup.Email; txtAddress.Text = sup.Address; }
            };
            ...
            btnAdd.Click += async (s, e) => {
                var sup = new Supplier { Name = txtName.Text, Phone = txtPhone.Text, Email = txtEmail.Text, Address = txtAddress.Text };
                var (ok, err) = await _svc.AddAsync(sup);
                if (ok) { ClearInputs(); await LoadAsync(); } else UIHelper.ShowError(err);
            };
            btnUpdate.Click += async (s, e) => {
                if (_selectedId == 0) return;
                var sup = new Supplier { Id = _selectedId, Name = txtName.Text, Phone = txtPhone.Text, Email = txtEmail.Text, Address = txtAddress.Text };
                var (ok, err) = await _svc.UpdateAsync(sup);
                if (ok) { ClearInputs(); await LoadAsync(); } else UIHelper.ShowError(err);
            };
